...
export default function TableAssignPlanWeek(props: PropsTable) {
    const { contentSearch,
        title,
        handleOpenCard,
        handleOpenEditCard,
    ...
    const { dataPlanMonth } = usePlanMonth()
    ...
    const [data, setData] = useState<PlanWeek[]>([]);
    ...
    const visibleRows = useMemo(() => {
        ...
        return data
    }, [page, data, rowsPerPage, contentSearch]);

    const isSelected = (id: number) => selected.indexOf(id) !== -1;

    const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
        if (event.target.checked) {
            const newSelected = visibleRows?.map((row) => row?.tuanID);
    ...
    const handleViewRole = (row: PlanWeek) => {
        if (row?.createBy === userName) return true;
        return false
    }
                        ...
                        >
                            ...
                            <TableBody>
                                {visibleRows.map((row, index) => {
                                    const isItemSelected = isSelected(row.tuanID);
                                    const labelId = `enhanced-table-checkbox-${row.tuanID}`;
                                    return (
                                        ...
                                        >
                                            {/* {!row?.isApprove &&
                                                <StyledTableCell padding="checkbox">
                                                    <Checkbox
                                                        color="primary"
                                                        onClick={(event) => handleClick(event, row.tuanID)}
                                            ...
                                            <StyledTableCell key={index} align="left">{row?.tuan_Thang ? row?.tuan_Thang : 'Chưa có dữ liệu'}</StyledTableCell>
                                            <StyledTableCell key={index} align="left">{row?.thangID ? dataPlanMonth.find(item => item.thangID === item?.thangID)?.thang : 'Chưa có dữ liệu'}</StyledTableCell>
                                                ...
                                                <Box width={"100%"} display='flex' gap={1} alignItems='center' justifyContent='flex-start'>
                                                    {isButtonView &&
                                                        <Avatar variant="rounded" sx={{ bgcolor: theme.palette.primary.main }}>
                                                            <IconEye onClick={() => handleViewItem(row.tuanID)} stroke={1.5} />
                                                        </Avatar>
                                                    }
                                                    {isButtonEdit &&
                                                        <Avatar variant="rounded" sx={{ bgcolor: "green" }}>
                                                            <IconPencil onClick={() => handleEditItem(row.tuanID)} stroke={1.5} />
                                                        </Avatar>
                                                    }
                                                    {!row?.isApprove &&
                                                        <Avatar variant="rounded" sx={{ bgcolor: theme.palette.primary.main }}>
                                                            <IconTrash onClick={(event) => handleClick(event, row.tuanID)} stroke={1.5} />
                                                        </Avatar>
                                                    }
                                                </Box>
                                            </StyledTableCell>
