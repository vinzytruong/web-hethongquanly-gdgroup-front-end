...
import { Button, Checkbox, Chip, ClickAwayListener, Fade, Grid, IconButton, Paper, Popper, PopperPlacementType, Switch, TableHead, TableRow, Typography } from '@mui/material';
import { IconChevronRight, IconDotsVertical, IconEye, IconInfoSquare } from '@tabler/icons-react';
...
export default function TablePlanMonth(props: PropsTable) {
    ...
    const handleConfirm = async (event: React.ChangeEvent<HTMLInputElement>, thangID: number, isApprove: boolean) => {
        const rs = await confirmPlanMonth(thangID, isApprove)
    ...
    const infoByID: PlanMonth = visibleRows.find(item => item.thangID === viewID)
    const isDisabled = infoByID?.nguoiDuyetID !== userID || infoByID?.isApprove

    console.log("dsdsd", infoByID);
                ...
                <Box sx={{ borderRadius: '6px', width: '100%', display: "table", tableLayout: "fixed", backgroundColor: theme.palette.background.paper }}>
                    ...
                    <TableContainer>
                        ...
                        >
                            ...
                            <TableBody>
                                {visibleRows.map((row: PlanMonth, index) => {
                                    return (
                                        <TableRow
                                            hover
                                            role="checkbox"
                                            ...
                                            sx={{ cursor: 'pointer' }}
                                        >
                                            <StyledTableCell align="left">Tháng {row.thang ? row.thang : 'Chưa có dữ liệu'}</StyledTableCell>
                            ...
                            <CustomDialog
                                ...
                                content={
                                    <Grid container spacing={2}>
                                        ...
                                        <Grid item sm={6}>
                                            <Typography><span style={{ fontWeight: "bolder" }}>Tháng:</span> {infoByID?.thang}</Typography>
                                        </Grid>
                                        <Grid item sm={6}>
                                                ...
                                                <Box sx={{
                                                    ...
                                                    <Box>
                                                        <Button
                                                            ...
                                                            onClick={(e: any) => handleConfirm(e, infoByID.thangID, true)}
                                                        >
                                                            Duyệt
                                                        </Button>
                                                    </Box>
                                                    <Box>
                                                        <Button
                                                            variant='contained'
                                                            onClick={(e: any) => handleConfirm(e, infoByID.thangID, false)}
                                                            sx={{
                                                                backgroundColor: "red",
                                                                boxShadow: "none",
                                                                '&:hover': {
