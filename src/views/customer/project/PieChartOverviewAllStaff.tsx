...
const PieChartOverviewAllStaff = ({ dataAllStaff, dataReportProjectInteract }: Props) => {
    const theme = useTheme()
    const router = useRouter()
    const elementRef = useRef<any>(null);
console.log("allstaff",dataAllStaff);
    ...
    const dataExpectedValue = useMemo(() => {
        let data: any = []
        dataAllStaff?.map((staff, index) => {
            data.push({
                id: index,
                value: countExpectedValueByStaff(staff?.nhanVienID),
                label: staff?.tenNhanVien,
                color:generateContrastingColors(dataAllStaff?.length)?.[index]
            })
        })
        ...
        <Grid container spacing={2}>
            <Grid item xs={12}>
                <Box sx={{ border: 1, borderColor: theme.palette.grey[400], borderRadius: '8px', p: 3 }}>
                    <Typography variant="h6">Cơ cấu giá trị cơ hội tất cả nhân viên</Typography>
                    <PieChartResponsive
            ...
            <Grid item xs={12}>
                <Box sx={{ border: 1, borderColor: theme.palette.grey[400], borderRadius: '8px', p: 3 }}>
                    <Typography variant="h6">Cơ cấu doanh thu tất cả nhân viên</Typography>
                    <PieChartResponsive
...
export default PieChartOverviewAllStaff